...
import { Trash2, Waves, Clock, CheckCircle, XCircle } from "lucide-react";
import type { ReactElement } from "react";

type PoolNodeData = Node<{
  label: string;
  status: "idle" | "running" | "success" | "error";
}>;


export function PoolNode({ data, id }: NodeProps<PoolNodeData>) {
  const { setNodes } = useReactFlow();
  const statusColor:Record<string,string> = {
    idle: "border-amber-600",
